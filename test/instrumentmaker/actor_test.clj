...
(deftest commits-a-registered-work-log
  (let [st (fresh-store)
        ...
        request {:worker-id "worker-1" :op :log-work-record :stake :low
                  :workshop-id "WS-1" :task "watch-movement inspection progress log"}
...
(deftest holds-an-unregistered-workshop-proposal
  (let [st (fresh-store)
        graph (actor/build-graph {:store st})
        request {:worker-id "worker-1" :op :log-work-record :stake :low
                  :workshop-id "WS-ghost" :task "watch-movement inspection progress log"}
...
(deftest interrupts-then-approves-safety-concern-on-human-approval
  (let [st (fresh-store)
        ...
        request {:worker-id "worker-1" :op :flag-safety-concern :stake :low
                  :workshop-id "WS-1" :hazard-type :fine-detail-eye-strain-hazard}
...
(deftest holds-a-scope-excluded-op-even-at-high-confidence
  (testing "an actor run can never commit a proposal that would finalize an instrument-repair/calibration-execution decision, regardless of disposition path"
    (let [st (fresh-store)
          graph (actor/build-graph {:store st})
          request {:worker-id "worker-1" :op :finalize-instrument-repair-decision :stake :low
                    :workshop-id "WS-1" :task "instrument-repair decision"}
