(ns instrumentmaker.advisor
  "Instrument Maker Advisor — proposing a precision-instrument workshop
  ...
  precision-parts/materials supply order) from a crew roster, workshop
  registration and safety-reporting policy. Swappable mock/llm; the
  advisor ONLY proposes — `instrumentmaker.governor` independently
  gates every proposal and always escalates safety concerns and
  above-threshold supply orders. The advisor never proposes to
  directly finalize an instrument-repair/calibration-execution
  decision (e.g. a specific instrument-repair or calibration
  procedure) or to override a workshop safety officer's judgment —
  those stay permanently out of this actor's scope. Modeled on
  cloud-itonami-isco-7222's advisor for the physical-safety-domain
  shape.
...
(defn- rationale-for [op worker-id workshop-id hazard-type]
  (case op
    ...
    (str "scheduled crew operation for instrument-repair task at workshop " workshop-id)
...
(def ^:private system-prompt
  "You are a precision-instrument workshop scheduling/logistics
   ...
   propose to directly finalize an instrument-repair/calibration-
   execution decision (e.g. deciding to proceed with a specific
   instrument-repair or calibration procedure), or to override a
   workshop safety officer's judgment — those are always out of this
   actor's scope; it coordinates workshop scheduling/logistics only
   and never performs instrument-repair or calibration work itself.
