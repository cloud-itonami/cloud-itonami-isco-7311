(ns instrumentmaker.governor
  ...
  a precision-instrument workshop crew. The governor never dispatches
  hardware itself, never performs instrument-repair or calibration
  work on the shop floor, and never finalizes an instrument-repair/
  calibration-execution decision (e.g. deciding to proceed with a
  specific instrument-repair or calibration procedure) or overrides a
  ...
  instrument-repair or calibration work itself). Modeled on
  ...
  HARD invariants (:hard? true, ALWAYS :hold, never overridable):
    ...
    3. no-actuation           — proposal :effect must be :propose (the
                                ...
                                never performs instrument-repair or
                                calibration work itself; it only gates
    ...
    5. scope-excluded action  — any proposal to directly finalize an
                                instrument-repair/calibration-execution
                                decision (e.g. deciding to proceed with
                                a specific instrument-repair or
                                calibration procedure), or to override a
                                workshop safety officer's judgment, is a
                                ...
                                calibration procedure\" / \"authorize
                                the instrument-repair operation\" /
                                \"override the workshop safety
                                officer's judgment\", never as bare
                                nouns like \"instrument\",
                                \"calibration\" or \"safety\", so the
                                ...
                                instrument-repair task …\" or \"…routed
                                for workshop safety officer review\" —
  ...
  regardless of confidence):
    6. :op :flag-safety-concern (an eye-strain/hand-injury/solder- or
                                chemical-exposure concern always
...
(def ^:private scope-excluded-ops
  #{:finalize-instrument-repair-decision :authorize-calibration-operation
    :proceed-with-calibration-procedure :override-safety-officer-judgment
    :override-workshop-safety-officer-judgment})

;; Full finalization/execution ACTION phrases only — never bare nouns
;; ("instrument", "calibration", "repair", "safety", "workshop safety
...
;; nouns, e.g. "instrument-repair task" / "workshop safety officer
;; review"). See
;; `governor-test/default-mock-advisor-proposals-never-self-trip-on-scope-exclusion`.
(def ^:private scope-excluded-phrases
  ["proceed with the instrument-repair operation" "proceed with the instrument repair operation"
   "proceed with the calibration procedure" "proceed with the calibration work"
   "finalize the instrument-repair decision" "finalize the calibration decision"
   "authorize the instrument-repair operation" "authorize the calibration operation"
  ...
  (let [{:keys [op rationale]} proposal]
    (cond-> []
      ...
      (conj {:rule :scope-excluded-action
             :detail "精密機器の修理・校正の実行判断の確定・workshop safety officer の判断の上書きは、この actor の権限外 — 常に永続ブロック"}))))
